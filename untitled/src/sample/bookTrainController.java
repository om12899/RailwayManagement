...
public class bookTrainController implements Initializable {
    int av1=0;
    String reID;
    ResultSet rs;
    @FXML
    Button BackCancelTicket;
    @FXML TextField txtTrId;
    @FXML
    TableView TrainSearch;
    @FXML
    TableColumn<Train,String> trainname;
    ...
    @FXML
    int trainID;
    public  void setBtnSearch(ActionEvent event)throws  SQLException
    {
        {
            ...
            else
            {
                ...
                if(trainID!=0) {
                    ...
                    confirmbookingController confirmbookingController=new confirmbookingController();
                    reID=Integer.toString(trainID);
    ...
    @FXML Button go;
    public void setGo(ActionEvent event)
    {
        try {
            Parent root1 = FXMLLoader.load(getClass().getResource("details.fxml"));
            Stage window = (Stage)((Node)event.getSource()).getScene().getWindow();
    ...
    @FXML
    public String trainid1;
    public void setBtnBook(ActionEvent event)throws SQLException
    {
       trainid1=txtTrId.getText();
        int abx=Integer.parseInt(trainid1);
       if(trainid1.equals(""))
       {
           Stage secondaryStage = new Stage();
           Parent root1 = null;
           try {
               root1 = FXMLLoader.load(getClass().getResource("emptyID.fxml"));
           } catch (IOException f) {
           }
       ...
       else if(abx>0&&abx<=45)
       {
           String sqlsearch = "SELECT * FROM train1 WHERE trainid='" + trainid1 + "'";
           rs = Datasource.dbExecute(sqlsearch);
           while (rs.next())
           {
               av1=rs.getInt("availability");
               ...
               if(av1!=0)
               {
                   ...
                   try {
                       ...
                       confirmbookingController controller = loader.getController();
                       controller.setId(trainid1);
                       Stage window = (Stage)((Node)event.getSource()).getScene().getWindow();
                       window.setTitle("Confirm your Booking");
                       window.setScene(new Scene(root1, 600, 400));
                       window.show();
               ...
               else
               {
                   ...
                   Parent root1 = null;
                   try {
                       root1 = FXMLLoader.load(getClass().getResource("full.fxml"));
                   } catch (IOException f) {
                   }
                   secondaryStage.setTitle("Error");
                   secondaryStage.setScene(new Scene(root1, 316, 66));
